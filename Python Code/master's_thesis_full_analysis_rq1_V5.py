# -*- coding: utf-8 -*-
"""
Created on Fri May 17 09:14:08 2024
...
# Traverse subfolders and date-specific subfolders within the directory containing IMU data
    ...
    if os.path.isdir(subfolder_path):
        ...
        for date_folder in os.listdir(subfolder_path):
            ...
            if os.path.isdir(date_folder_path):
                # Walk through files within the date-specific subfolders, specifically looking for CSV files
...
# Combine the "First Name" and "Last Name" columns in the imu_data DataFrame into a new "Name" column
...
# Columns to remove from post_data
columns_to_remove = [
...
positions = {
    'Brendan Amoyaw': 'Forward', 'Cashius McNeilly': 'Guard', 'Daniel Graham': 'Guard', 'Elijah Bethune': 'Guard',
    'Jeremiah Francis': 'Guard', 'Kazim Raza': 'Guard', 'Matthew Groe': 'Guard', 'Mike Demagus': 'Guard',
    'Moody Muhammed': 'Forward', 'Parker Davis': 'Guard', 'Riaz Saliu': 'Forward', 'Sebastian Di Manno': 'Guard',
    'Stevan Japundzic': 'Forward', 'Thomas Mattsel': 'Forward'
}

# Map positions to the data
...
data_long['PlayerID'] = data_long['Name'].map(player_ids)

# Drop the Name column
data_long.drop(columns='Name', inplace=True)

# Display the first few rows of the reshaped dataframe
data_long.head()

# Write the final DataFrame to an Excel file
#data_long.to_excel('linear_mixed_model_data.xlsx', index=False)
