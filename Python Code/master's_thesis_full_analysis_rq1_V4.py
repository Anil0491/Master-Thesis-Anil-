...
def plot_individual_metric_comparison(results, title_base, colors=('skyblue', 'salmon')):
    for metric, data in results.items():
        ...
        fig, ax = plt.subplots(figsize=(10, 12))  # Ensuring a fresh figure is created each time
        ...
        width = 0.4  # Adjusted width for better 
        ...
        ax.set_ylabel('Values', fontsize=14, fontweight='bold')  # Bigger and bold y-axis label
        ax.set_xlabel('Metrics', fontsize=14, fontweight='bold')  # Bigger and bold x-axis label (if needed)
        ax.set_title(f'{title_base}: {metric_name}', fontsize=15, pad=25, fontweight='bold')

        ax.set_xticks(x)
        ax.set_xticklabels([metric_name], fontsize=12, fontweight='bold')  # Bigger and bold x-tick labels
        ax.set_yticklabels(ax.get_yticks(), fontsize=12, fontweight='bold')  # Bigger and bold y-tick labels

        # Adding significance label
        significance = "*" if p_value < 0.05 else "ns"
        y_height = max(pre_mean, post_mean)
        ax.text(x, y_height + 0.001 * y_height, significance, ha='center', va='bottom',fontsize=30)
