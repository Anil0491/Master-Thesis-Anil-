...
#excel_file_path = 'correlation_results.xlsx'

# Save the DataFrame to an Excel file
#correlation_df.to_excel(excel_file_path, index=False)
...
# Calculate average values for each 'Name'
average_values = force_plate.groupby('Name').mean().reset_index()

# Define threshold values for each quadrant
threshold_x = average_values['Concentric Peak Force / BM [N/kg] '].mean()
threshold_y = average_values['Eccentric Peak Force / BM [N/kg] '].mean()

# Determine the quadrant for each 'Name' based on average values
average_values['Quadrant'] = None
average_values.loc[
    (average_values['Concentric Peak Force / BM [N/kg] '] >= threshold_x) &
    (average_values['Eccentric Peak Force / BM [N/kg] '] >= threshold_y), 'Quadrant'] = 'Q1'
average_values.loc[
    (average_values['Concentric Peak Force / BM [N/kg] '] < threshold_x) &
    (average_values['Eccentric Peak Force / BM [N/kg] '] >= threshold_y), 'Quadrant'] = 'Q2'
average_values.loc[
    (average_values['Concentric Peak Force / BM [N/kg] '] < threshold_x) &
    (average_values['Eccentric Peak Force / BM [N/kg] '] < threshold_y), 'Quadrant'] = 'Q3'
average_values.loc[
    (average_values['Concentric Peak Force / BM [N/kg] '] >= threshold_x) &
    (average_values['Eccentric Peak Force / BM [N/kg] '] < threshold_y), 'Quadrant'] = 'Q4'

# Plot the quadrant chart and label each point with 'Name'
for index, row in average_values.iterrows():
    plt.scatter(row['Concentric Peak Force / BM [N/kg] '], row['Eccentric Peak Force / BM [N/kg] '], label=row['Name'])


# Add grid lines to divide the quadrants
plt.axvline(x=threshold_x, color='black', linestyle='--')
plt.axhline(y=threshold_y, color='black', linestyle='--')


# Add labels to the quadrants
plt.text(threshold_x - 2, threshold_y + 2, 'Q1', fontsize=12, color='green')
plt.text(threshold_x - 2, threshold_y - 4, 'Q2', fontsize=12, color='blue')
plt.text(threshold_x - 2, threshold_y - 6, 'Q3', fontsize=12, color='red')
plt.text(threshold_x + 2, threshold_y + 2, 'Q4', fontsize=12, color='orange')

plt.xlabel('Concentric Peak Force / BM [N/kg] ')
plt.ylabel('Eccentric Peak Force / BM [N/kg] ')
plt.title('Quadrant Chart')

# Label each point with 'Name'
          for index, row in average_values.iterrows():x
plt.annotate(row['Name'], (row['Concentric Peak Force / BM [N/kg] '], row['Eccentric Peak Force / BM [N/kg] ']))
