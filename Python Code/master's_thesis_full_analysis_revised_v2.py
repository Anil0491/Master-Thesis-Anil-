# -*- coding: utf-8 -*-
"""
Created on Sat Jul 13 08:43:11 2024
...
# Define the force plate metrics
force_plate_metrics = [
...
for metric in force_plate_metrics:
    ...
    if result is not None:
        mixed_model_results.append(result)
...
#next steps in the analysis is to see if average practice volume (impact load and average intensity) has effect on the average change of each of the force plate metrics. 
...
# Perform mixed model analysis for each metric change with separate fixed effects
fixed_effects_list = {
    'Avg_Impact_Load': ['Avg_Impact_Load'],
    'Avg_Average_Intensity': ['Avg_Average_Intensity'],
...
for metric in metric_columns:
    ...
    result_summary_load = fit_mixed_model(long_format_change_data[long_format_change_data['Metric'] == metric], 'Change', fixed_effects_list['Avg_Impact_Load'], 'PlayerID')
    impact_load_results[metric] = result_summary_load
    
    # Analysis with Avg_Average_Intensity
    result_summary_intensity = fit_mixed_model(long_format_change_data[long_format_change_data['Metric'] == metric], 'Change', fixed_effects_list['Avg_Average_Intensity'], 'PlayerID')
    average_intensity_results[metric] = result_summary_intensity
    
   

# Print the summary of mixed model results for each metric
for metric in metric_columns:
    print(f"Mixed Model Results for {metric} Change with Avg Impact Load:\n{impact_load_results[metric]}\n")
    print(f"Mixed Model Results for {metric} Change with Avg Average Intensity:\n{average_intensity_results[metric]}\n")
...
# Prepare the force plate metrics
force_plate_metrics = [
    'Jump Height (Imp-Mom) in Inches [in] ',
    'RSI-modified (Imp-Mom) [m/s] ',
    'Eccentric Braking RFD / BM [N/s/kg] ',
...
def plot_metric_changes(metric, predictions_load, predictions_intensity):
    ...
    sns.lineplot(x='Avg_Impact_Load', y='Predicted', data=predictions_load, color='red', linewidth=2, label='Regression Line')
    ...
    sns.lineplot(x='Avg_Average_Intensity', y='Predicted', data=predictions_intensity, color='red', linewidth=2, label='Regression Line')
